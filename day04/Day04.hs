...
match v ch | value ch == v = choose ch
           | otherwise     = ch
...
winner [] = (Nothing, [])
winner (b : bg) | isJust (winningRow b) = (Just b, bg)
                | otherwise             = (mbg, b : rest)
  where (mbg, rest) = winner bg
...
parse str = (parseDrawList dl, parseBingos ss) where (dl : ss) = lines str
...
parseBingos ss = filter isBingo $ map parseBingo bgss
  where bgss = splitOn [""] ss

parseBingo :: [String] -> Bingo
parseBingo ss = Bingo $ map (map (choice . read)) nss where nss = map words ss
